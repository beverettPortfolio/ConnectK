...
public class BoardEvaluator {
	...
	public static int evaluate(BoardModel searchState, byte player){
		...
		if (winner==-1){
			...
			//Searches every space for winning lines and threats that start on that space
			for(int i=0; i<searchState.getWidth(); ++i){
				for(int j=0; j<searchState.getHeight(); ++j){
					...
					//Vertical. Threats are not counted for vertical winning lines
	 ...
	 * @param searchState The board state to be searched
	 * @param row The row to start on
	 * @param column The column to start on
	 * @param k The amount needed in a row to win
	 * @param horizontalMovement What direction to move horizontally. 1 for right, 0 for vertical only, -1 for left
	 * @param verticalMovement What direction to move vertically. 1 for up, 0 for horizontal only. Downward movement is not needed.
	...
	private static HashMap<Heuristics, Integer> searchRow(BoardModel searchState, int row, int column, int k, int horizontalMovement, int verticalMovement){
		...
		if (row+(k*horizontalMovement)>=-1 && row+(k*horizontalMovement)<=searchState.getWidth() && column+(k*verticalMovement)<=searchState.getHeight()){
			...
			if (count1>0&&count2==0){
				...
				//Threats are checked for lines with horizontal movement when gravity is on
